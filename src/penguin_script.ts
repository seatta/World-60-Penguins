const penguin_site: string = `https://jq.world60pengs.com`;
const penguin_site_json: string = `${penguin_site}/rest/cache/actives.json`;
...
/**
 * Opens the world60pengs site in the browser
 */
...
async function refresh() {
  const now = new Date();
  if (perform_fetch) penguin_data = await fetch_penguin_data(`${cors_url}?_=${now}`);
